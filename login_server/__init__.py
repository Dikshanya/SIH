...
def create_app(test_config=None):
    ...
    def get_weather():
        ...
        start_date = str(datex.month-1)+'-'+str(datex.day)+'-'+str(datex.year)
    ...
    def register():
        if request.method == 'POST':
            ...
            if error is None:
                ...
                db.commit()
                return jsonify({"status_code": "200", "message": "Registration successful"})
    ...
    def login():
        if request.method == 'POST':
            ...
            if error is None:
                token = ''.join(choice(ascii_letters)for i in range(12))
                db.execute(
                    'Update user SET token = ? WHERE username = ?',
                    (token, user["username"])
                )
                db.commit()
                return jsonify({"status_code": 200, "message": token})

        return jsonify({"status_code": 200, "message": error})
